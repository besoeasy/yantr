...
import { HardDrive, Layers, Trash2, ArrowUpRight } from '@lucide/vue'
    ...
    <div class="relative z-10 flex flex-col gap-4">
      <div class="flex items-start justify-between">
        ...
        <div class="flex flex-col items-end">
          <span class="text-lg font-black text-zinc-900 dark:text-white">{{ remaining }}</span>
          <span class="text-[9px] font-semibold uppercase tracking-widest text-zinc-500">
            {{ t('quickMetrics.biggestUnused.remaining') }}
          </span>
        </div>
      </div>
    </div>
    ...
    <div class="relative z-10 mt-auto flex flex-col pt-6">
      <Transition name="flip" mode="out-in">
        <div :key="`${current.kind}-${current.id}-${flipKey}`" class="flex flex-col">
          <div class="mb-1 flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-widest text-zinc-500">
            <component
              :is="current.kind === 'image' ? Layers : HardDrive"
              class="h-3.5 w-3.5"
            />
            <span>{{ current.kind === 'image' ? t('quickMetrics.biggestUnused.image') : t('quickMetrics.biggestUnused.volume') }}</span>
          </div>

          <div class="mb-2 tabular-nums text-4xl font-black tracking-tighter text-zinc-900 transition-transform duration-500 group-hover:-translate-y-1 sm:text-5xl dark:text-white">
            {{ formatBytes(current.sizeBytes) }}
          </div>

          <div class="mb-6 truncate text-xs font-semibold text-zinc-500 dark:text-zinc-400" :title="current.name">
            {{ current.name }}
          </div>
          ...
          <div class="flex items-center gap-2">
            <button
              ...
              class="group/btn flex flex-1 items-center justify-center gap-2 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-red-600 transition-colors hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-60 dark:border-red-900/40 dark:bg-red-900/20 dark:text-red-400 dark:hover:bg-red-900/30"
            >
              <Trash2 class="h-3.5 w-3.5" :class="{ 'animate-pulse': deleting }" />
              {{ deleting ? t('quickMetrics.biggestUnused.deleting') : t('quickMetrics.biggestUnused.delete') }}
            </button>

            <button
              type="button"
              @click.stop="goToManage"
              :title="t('quickMetrics.biggestUnused.viewAll')"
              class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-zinc-200 bg-zinc-50 text-zinc-600 transition-colors hover:border-zinc-900 hover:bg-zinc-900 hover:text-white dark:border-zinc-800 dark:bg-zinc-900/50 dark:text-zinc-400 dark:hover:border-zinc-100 dark:hover:bg-zinc-100 dark:hover:text-zinc-900"
            >
